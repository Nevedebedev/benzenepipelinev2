...
class BenzenePipeline:
    ...
    def apply_nn2_correction(self, pinn_pred_ppb, current_sensors_ppb, met_data):
        ...
        FIXED: Removed current_sensors input - new model architecture doesn't use it.
        """
        try:
            # Handle potential NaNs upfront
            pinn_pred_ppb = np.nan_to_num(pinn_pred_ppb, nan=0.0)
            # FIXED: current_sensors_ppb parameter kept for backward compatibility but not used
            
            # 1. Scale Inputs
            # FIXED: Handle zeros the same way as training (mask before scaling)
            pinn_nonzero_mask = pinn_pred_ppb != 0.0
            p_s = np.zeros_like(pinn_pred_ppb)
            if pinn_nonzero_mask.any():
                p_s[pinn_nonzero_mask] = self.nn2_scalers['pinn'].transform(
                    pinn_pred_ppb[pinn_nonzero_mask].reshape(-1, 1)
                ).flatten()
            p_s = p_s.reshape(1, -1)
            
            # FIXED: Removed s_s (current_sensors) - not part of new model architecture
            ...
            final_pred = corrected_ppb.cpu().numpy().flatten()
