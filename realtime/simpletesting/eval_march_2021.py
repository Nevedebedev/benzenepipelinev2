"""
Apply NN2 correction to March 2021 PINN predictions and evaluate
"""
...
pinn_path = os.path.join(BASE_DIR, "sensors_pinn_benzene_ppb_2021_march.csv")
df_pinn = pd.read_csv(pinn_path)
df_pinn['t'] = pd.to_datetime(df_pinn['t'])

# Load ground truth
print("Loading ground truth...")
gt_path = os.path.join(MADIS_DIR, "results_2021/sensors_actual_wide_2021_full_march.csv")
df_gt = pd.read_csv(gt_path)
df_gt['t'] = pd.to_datetime(df_gt['t'])

# Load meteorology
print("Loading meteorology...")
met_path = os.path.join(MADIS_DIR, "training_data_2021_march_REPAIRED/BASF_Pasadena_training_data.csv")
...
if len(df_valid) > 0:
    ...
    # Save results
    output_path = os.path.join(BASE_DIR, "results_march_2021_hybrid.csv")
    df_results.to_csv(output_path, index=False)
    print(f"\n✓ Detailed results saved to: {output_path}")
    
    # Save wide format for user
    wide_df = df_results.pivot(index='timestamp', columns='sensor_id', values='final_pred')
    wide_df.to_csv(os.path.join(BASE_DIR, "march_test_1ppb.csv"))
    ...
    with open(log_path, 'a') as f:
        ...
        f.write(f"| {timestamp} | March 2021 (Fixed PINN) | {len(df_valid)//9} hrs | "
               f"MAE PINN: {mae_pinn:.4f} | MAE Hybrid: {mae_hybrid:.4f} | "
               f"Real-time PINN + NN2 |\n")
    
    print(f"✓ Logged to resultslog.md")
    print(f"✓ Saved wide format to: march_test_1ppb.csv")
