"""
Apply NN2 correction to January 2021 PINN predictions and evaluate
"""
...
pinn_path = os.path.join(BASE_DIR, "sensors_pinn_benzene_ppb_2021_jan.csv")
df_pinn = pd.read_csv(pinn_path)
df_pinn['t'] = pd.to_datetime(df_pinn['t'])

# Load ground truth
print("Loading ground truth...")
gt_path = os.path.join(MADIS_DIR, "results_2021/sensors_actual_wide_2021_full_jan.csv")
df_gt = pd.read_csv(gt_path)
df_gt['t'] = pd.to_datetime(df_gt['t'])

# Load meteorology (from one facility for now - should ideally be unified)
print("Loading meteorology...")
met_path = os.path.join(MADIS_DIR, "training_data_2021_full_jan_REPAIRED/BASF_Pasadena_training_data.csv")
...
if len(df_valid) > 0:
    ...
    # Save results
    output_path = os.path.join(BASE_DIR, "results_jan_2021_hybrid.csv")
    ...
    with open(log_path, 'a') as f:
        ...
        f.write(f"| {timestamp} | Jan 2021 (Fixed PINN) | {len(df_valid)//9} hrs | "
