"""
NN2 Training Script for Colab - Clean Version (Data Already Downloaded)

This script assumes all data has already been downloaded to /content/data/
...
# ═══════════════════════════════════════════════════════════════════
# 2. NN2 NETWORK - FIXED (NO DATA LEAKAGE)
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# 5. LEAVE-ONE-SENSOR-OUT CROSS-VALIDATION
# ═══════════════════════════════════════════════════════════════════

def leave_one_sensor_out_cv():
    """Train NN2 with leave-one-sensor-out cross-validation"""
    
    print("="*80)
    print("NN2 TRAINING - LEAVE-ONE-SENSOR-OUT CROSS-VALIDATION")
    print("="*80)
    ...
    # Train on each sensor being held out
    for sensor_idx in range(CONFIG['n_sensors']):
        sensor_id = full_dataset.all_sensor_ids[sensor_idx]
        print("="*80)
        print(f"Training with sensor {sensor_id} held out (sensor {sensor_idx+1}/{CONFIG['n_sensors']})")
        print("="*80)
        
        # Create train/val split
        train_dataset = BenzeneDataset(
            data_path=CONFIG['sensor_file'],
            source_dir=CONFIG['source_dir'],
            pinn_path=CONFIG['pinn_file'],
            sensor_coords_path=CONFIG['sensor_coords_file'],
            held_out_sensor_idx=sensor_idx,
            scalers=scalers,
            fit_scalers=False
        )
        
        val_dataset = BenzeneDataset(
            data_path=CONFIG['sensor_file'],
            source_dir=CONFIG['source_dir'],
            pinn_path=CONFIG['pinn_file'],
            sensor_coords_path=CONFIG['sensor_coords_file'],
        ...
        for epoch in range(CONFIG['epochs']):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(checkpoint, f"{CONFIG['save_dir']}/nn2_sensor_{sensor_idx}.pth")
            else:
                patience_counter += 1
        ...
        results[sensor_id] = eval_results
        
        print(f"\nResults for {sensor_id}:")
        print(f"  PINN MAE: {eval_results['pinn_mae']:.4f} ppb")
        print(f"  NN2 MAE:  {eval_results['nn2_mae']:.4f} ppb")
        print(f"  Improvement: {eval_results['improvement']:.1f}%\n")
    
    # Save results
    results_file = f"{CONFIG['save_dir']}/leave_one_out_results.json"
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save scalers
    scalers_file = f"{CONFIG['save_dir']}/nn2_scalers.pkl"
...
# ═══════════════════════════════════════════════════════════════════
# 6. MASTER MODEL TRAINING (ALL SENSORS)
# ═══════════════════════════════════════════════════════════════════

def train_master_model(scalers):
    """Train master model on all sensors (no holdout)"""
    
    print("="*80)
    print("NN2 TRAINING - MASTER MODEL (ALL SENSORS)")
    print("="*80)
    print(f"\nDevice: {CONFIG['device']}")
    print(f"Save directory: {CONFIG['save_dir']}\n")
    ...
    # Create full dataset (no holdout)
    ...
    full_dataset = BenzeneDataset(
        ...
        pinn_path=CONFIG['pinn_file'],
        sensor_coords_path=CONFIG['sensor_coords_file'],
        held_out_sensor_idx=None,  # No holdout - use all sensors
        scalers=scalers,
        fit_scalers=False
    )
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(checkpoint, f"{CONFIG['save_dir']}/nn2_master_model.pth")
        else:
            patience_counter += 1
            if patience_counter >= max_patience:
                print(f"Early stopping at epoch {epoch+1}")
    ...
    print(f"\nAverage improvement: {avg_improvement:.1f}%")
    print(f"Master model saved to: {CONFIG['save_dir']}/nn2_master_model.pth")
    print("="*80)
...
if __name__ == '__main__':
    ...
    print("="*80)
    results, scalers = leave_one_sensor_out_cv()
    
    # Step 2: Train master model on all sensors
    print("\n" + "="*80)
    print("STEP 2: MASTER MODEL TRAINING")
    print("="*80)
    master_model, master_results = train_master_model(scalers)
    
    print("\n" + "="*80)
    print("✓ ALL TRAINING COMPLETE!")
    print("="*80)
    print(f"\nFiles saved:")
    print(f"  • Leave-one-out models: {CONFIG['save_dir']}/nn2_sensor_*.pth")
    print(f"  • Master model: {CONFIG['save_dir']}/nn2_master_model.pth")
    print(f"  • Scalers: {CONFIG['save_dir']}/nn2_scalers.pkl")
    print(f"  • Results: {CONFIG['save_dir']}/leave_one_out_results.json")
    print("="*80)
