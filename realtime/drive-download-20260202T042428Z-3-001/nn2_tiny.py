"""
NN2 Training Script - PHASE 1: Tiny Model with Strong Regularization

This script implements Phase 1 of the overfitting fix:
- TINY architecture: 36 → 32 → 9 (~1.2K params, 44x smaller than current)
- STRONG regularization: lambda_correction=0.1, dropout=0.5
- Goal: Better generalization, avoid overfitting

Expected Results:
- Training: +20-40% improvement (lower than current, but OK!)
- 2019 Validation: +10-20% improvement (better generalization)
- 2021 Validation: +0-10% improvement (some generalization)

Key Insight: Lower training performance with better validation is BETTER than
high training with poor validation!
"""
...
# ═══════════════════════════════════════════════════════════════════
# CONFIGURATION - PHASE 1 (TINY MODEL + STRONG REGULARIZATION)
# ═══════════════════════════════════════════════════════════════════
...
CONFIG = {
    ...
    'learning_rate': 1e-4,
    'epochs': 100,
    'lambda_correction': 0.1,      # 10x stronger than improved version (was 0.01)
    'lambda_large': 0.1,            # Keep same
...
# ═══════════════════════════════════════════════════════════════════
# 2. NN2 NETWORK - TINY ARCHITECTURE (PHASE 1)
# ═══════════════════════════════════════════════════════════════════
...
class NN2_CorrectionNetwork(nn.Module):
    def __init__(self, n_sensors=9, scaler_mean=None, scaler_scale=None, output_ppb=True):
        ...
        # PHASE 1: TINY ARCHITECTURE - 36 → 32 → 9 (~1.2K params)
        # Previous: 36 → 256 → 128 → 64 → 9 (~53K params)
        # This is 44x smaller to prevent overfitting
        # With only 5,173 training samples, tiny model is more appropriate
        self.correction_network = nn.Sequential(
            nn.Linear(36, 32),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(0.5),  # HIGH dropout (was 0.3) - strong regularization
...
# ═══════════════════════════════════════════════════════════════════
# 4. LOSS & TRAINING FUNCTIONS (IMPROVED)
# ═══════════════════════════════════════════════════════════════════

def correction_loss(pred, target, corrections, valid_mask, lambda_correction=0.1, 
                   lambda_large=0.1, target_ppb=None, pinn_ppb=None):
    """
    IMPROVED loss function with large correction penalty.
    ...
    Args:
        ...
        lambda_correction: Regularization weight (default: 0.1, PHASE 1 - STRONG)
        lambda_large: Penalty for corrections >> PINN (default: 0.1)
    ...
    # Regularization: penalize large corrections (STRONG in Phase 1)
...
def train_epoch(model, dataloader, optimizer, device, lambda_correction, lambda_large):
    model.train()
    total_loss = 0
    total_valid = 0
    ...
    for batch in dataloader:
        ...
        loss, loss_dict = correction_loss(pred, target, corrections, valid_mask, 
                                          lambda_correction, lambda_large, target_ppb, pinn_ppb)
...
# ═══════════════════════════════════════════════════════════════════
# 5. MASTER MODEL TRAINING (ALL SENSORS) - PHASE 1 VERSION
# ═══════════════════════════════════════════════════════════════════

def train_master_model():
    """Train master model on all sensors (no holdout, no LOOCV) - PHASE 1"""
    
    print("="*80)
    print("NN2 TRAINING - PHASE 1: TINY MODEL + STRONG REGULARIZATION")
    print("="*80)
    print(f"\nDevice: {CONFIG['device']}")
    print(f"Save directory: {CONFIG['save_dir']}")
    print(f"\nConfiguration:")
    print(f"  Architecture: 36 → 32 → 9 (~1.2K params, TINY)")
    print(f"  Epochs: {CONFIG['epochs']}")
    print(f"  Learning Rate: {CONFIG['learning_rate']}")
    print(f"  Lambda Correction: {CONFIG['lambda_correction']} (STRONG)")
    print(f"  Lambda Large: {CONFIG['lambda_large']}")
    print(f"  Dropout: 0.5 (HIGH)")
    ...
    print(f"  Architecture: TINY (44x smaller than previous)\n")
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, CONFIG['device'], 
                                CONFIG['lambda_correction'], CONFIG['lambda_large'])
        ...
        print(f"Epoch {epoch+1:3d}/{CONFIG['epochs']}: "
              ...
              f"Large Penalty={loss_dict.get('large_penalty', 0):.4f}")
        ...
        training_history.append({
            ...
            'large_penalty': loss_dict.get('large_penalty', 0)
        })
    ...
    model_path = Path(CONFIG['save_dir']) / 'nn2_master_model_ppb_tiny.pth'
    checkpoint = {
        ...
        'architecture': 'tiny',  # Mark as Phase 1 model
    ...
    scalers_path = Path(CONFIG['save_dir']) / 'nn2_master_scalers_tiny.pkl'
...
if __name__ == '__main__':
    ...
    print("="*80)
    print(f"\nFiles saved:")
    print(f"  • Master model: {CONFIG['save_dir']}/nn2_master_model_ppb_tiny.pth")
    print(f"  • Scalers: {CONFIG['save_dir']}/nn2_master_scalers_tiny.pkl")
    print("\nNext Steps:")
    print("  1. Test on 2019 validation data (precomputed PINN)")
    print("  2. If validation > 0% improvement, proceed to Phase 2")
    print("  3. If validation still < 0%, increase regularization further")
    print("="*80)
