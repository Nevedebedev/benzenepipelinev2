"""
NN2 Training Script - PHASE 1.5: Balanced Model (Goldilocks Solution)

This script implements Phase 1.5 - the "Goldilocks" solution:
- BALANCED architecture: 36 → 64 → 16 → 9 (~1,800 params)
  - More capacity than tiny (1,545 params)
  - Still much smaller than original (53K params)
  - Good data-to-param ratio: 2.3 samples/param
- MODERATE regularization:
  - lambda_correction = 0.005 (5x original, not 100x like Phase 1)
  - dropout = [0.3, 0.2] (moderate, not aggressive)
  - weight_decay = 5e-5 (light L2)

Expected Results:
- Training: +40-50% improvement (lower than 68%, but more realistic)
- 2019 Validation: +20-35% improvement (close to training = good generalization)
- 2021 Test: +10-25% improvement (some drop expected, but still positive)

Key Insight: Find the sweet spot between underfitting (Phase 1) and overfitting (original model)
"""
...
# ═══════════════════════════════════════════════════════════════════
# CONFIGURATION - PHASE 1.5 (BALANCED MODEL + MODERATE REGULARIZATION)
# ═══════════════════════════════════════════════════════════════════
...
CONFIG = {
    ...
    'batch_size': 32,
    'learning_rate': 1e-4,
    'weight_decay': 5e-5,          # Light L2 regularization (5x original)
    'epochs': 100,
    'lambda_correction': 0.005,    # Moderate (5x original 0.001, not 100x like Phase 1)
    'lambda_size_penalty': 0.05,   # Gentler penalty for large corrections
...
# ═══════════════════════════════════════════════════════════════════
# 2. NN2 NETWORK - BALANCED ARCHITECTURE (PHASE 1.5)
# ═══════════════════════════════════════════════════════════════════
...
class NN2_CorrectionNetwork(nn.Module):
    def __init__(self, n_sensors=9, scaler_mean=None, scaler_scale=None, output_ppb=True):
        ...
        # PHASE 1.5: BALANCED ARCHITECTURE - 36 → 64 → 16 → 9 (~1,800 params)
        # Phase 1 Tiny: 36 → 32 → 9 (~1,545 params) - too small, underfitting
        # Original: 36 → 256 → 128 → 64 → 9 (~53K params) - too large, overfitting
        # This is the Goldilocks solution: 3.4x more than Phase 1, 29x less than original
        # Good data-to-param ratio: 2.3 samples/param (5,173 samples / 1,800 params)
        self.correction_network = nn.Sequential(
            nn.Linear(36, 64),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.3),      # Moderate dropout (was 0.5 in Phase 1)
            nn.Linear(64, 16),
            nn.BatchNorm1d(16),
            nn.ReLU(),
...
# ═══════════════════════════════════════════════════════════════════
# 4. LOSS & TRAINING FUNCTIONS (BALANCED)
# ═══════════════════════════════════════════════════════════════════

def balanced_loss(pred, target, corrections, valid_mask, lambda_correction=0.005, 
                 lambda_size_penalty=0.05, target_ppb=None, pinn_ppb=None):
    """
    BALANCED loss function with moderate regularization.
    ...
    Args:
        ...
        lambda_correction: Regularization weight (default: 0.005, MODERATE - 5x original)
        lambda_size_penalty: Penalty for corrections > 50% of PINN (default: 0.05, GENTLER)
    ...
    if valid_pred.numel() > 0:
        mse_loss = F.mse_loss(valid_pred, valid_target)
    else:
        mse_loss = torch.tensor(0.0, device=pred.device)

    # Regularization: penalize large corrections (MODERATE in Phase 1.5)
    correction_reg = lambda_correction * (corrections ** 2).mean()
    
    # Size penalty: penalize corrections > 50% of PINN magnitude (GENTLER than Phase 1)
    ...
    if pinn_ppb is not None and target_ppb is not None:
        ...
        # Penalize if correction > 50% of PINN prediction (moderate threshold)
        large_correction_mask = (correction_ratio > 0.5) & valid_mask
        if large_correction_mask.any():
            # Gentler penalty for corrections > 50% of PINN
            size_penalty = lambda_size_penalty * torch.relu(correction_ratio[large_correction_mask] - 0.5).mean()
    
    # Direction penalty (keep existing, but lighter)
    ...
    if target_ppb is not None and pinn_ppb is not None:
        ...
        if wrong_direction.any():
            direction_penalty = 0.2 * torch.relu(-corrections_ppb[wrong_direction] * error[wrong_direction]).mean()
    
    total_loss = mse_loss + correction_reg + size_penalty + direction_penalty
...
def train_epoch(model, dataloader, optimizer, device, lambda_correction, lambda_size_penalty):
    model.train()
    total_loss = 0
    total_valid = 0
    ...
    for batch in dataloader:
        ...
        loss, loss_dict = balanced_loss(pred, target, corrections, valid_mask, 
                                       lambda_correction, lambda_size_penalty, target_ppb, pinn_ppb)
...
# ═══════════════════════════════════════════════════════════════════
# 5. MASTER MODEL TRAINING (ALL SENSORS) - PHASE 1.5 VERSION
# ═══════════════════════════════════════════════════════════════════

def train_master_model():
    """Train master model on all sensors (no holdout, no LOOCV) - PHASE 1.5"""
    
    print("="*80)
    print("NN2 TRAINING - PHASE 1.5: BALANCED MODEL + MODERATE REGULARIZATION")
    print("="*80)
    print(f"\nDevice: {CONFIG['device']}")
    print(f"Save directory: {CONFIG['save_dir']}")
    print(f"\nConfiguration:")
    print(f"  Architecture: 36 → 64 → 16 → 9 (~1,800 params, BALANCED)")
    print(f"  Epochs: {CONFIG['epochs']}")
    print(f"  Learning Rate: {CONFIG['learning_rate']}")
    print(f"  Weight Decay: {CONFIG['weight_decay']} (L2 regularization)")
    print(f"  Lambda Correction: {CONFIG['lambda_correction']} (MODERATE - 5x original)")
    print(f"  Lambda Size Penalty: {CONFIG['lambda_size_penalty']} (GENTLER)")
    print(f"  Dropout: [0.3, 0.2] (MODERATE)")
    ...
    print(f"  Architecture: BALANCED (3.4x Phase 1, 29x smaller than original)")
    print(f"  Data-to-param ratio: {len(full_dataset)/total_params:.1f} samples/param\n")
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, CONFIG['device'], 
                                CONFIG['lambda_correction'], CONFIG['lambda_size_penalty'])
        ...
        print(f"Epoch {epoch+1:3d}/{CONFIG['epochs']}: "
              ...
              f"Reg={loss_dict.get('correction_reg', 0):.4f}, "
              f"Size Penalty={loss_dict.get('size_penalty', 0):.4f}")
        print(f"  Corrections: mean={correction_stats['mean']:.4f}, "
              f"max={correction_stats['max']:.4f}, "
              f"std={correction_stats['std']:.4f} (scaled space)")
        ...
        if correction_stats['max'] > 1.0:
            print(f"  ⚠️  WARNING: Large corrections detected (max={correction_stats['max']:.2f})")
        ...
        training_history.append({
            ...
            'correction_reg': loss_dict.get('correction_reg', 0),
            'size_penalty': loss_dict.get('size_penalty', 0)
        })
    ...
    print("\n" + "="*80)
    print("Evaluating master model on all sensors...")
    print("="*80)
    all_results = {}
    ...
    model_path = Path(CONFIG['save_dir']) / 'nn2_master_model_ppb_balanced.pth'
    checkpoint = {
        ...
        'config': CONFIG,
        'sensor_coords': full_dataset.sensor_coords_array,
        'training_history': training_history,
        'architecture': 'balanced',  # Mark as Phase 1.5 model
    ...
    scalers_path = Path(CONFIG['save_dir']) / 'nn2_master_scalers_balanced.pkl'
...
if __name__ == '__main__':
    ...
    print("="*80)
    print(f"\nFiles saved:")
    print(f"  • Master model: {CONFIG['save_dir']}/nn2_master_model_ppb_balanced.pth")
    print(f"  • Scalers: {CONFIG['save_dir']}/nn2_master_scalers_balanced.pkl")
    print("\nNext Steps:")
    print("  1. Test on 2019 validation data (precomputed PINN)")
    print("  2. Target: +20-35% improvement (close to training = good generalization)")
    print("  3. If successful, proceed to Phase 2 or test on 2021 data")
    print("="*80)
