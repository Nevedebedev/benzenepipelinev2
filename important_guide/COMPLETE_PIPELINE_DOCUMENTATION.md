...
4. [NN2 Training Process](#nn2-training-process)
5. [Real-Time Pipeline](#real-time-pipeline)
6. [Validation Process](#validation-process)
7. [Key Constants and Parameters](#key-constants-and-parameters)
...
2. **NN2 (Correction Network)**: Corrects PINN predictions using real sensor data

**Pipeline Flow**:
```
Meteorological Data → PINN (at sources) → Superimpose → Convert to ppb → NN2 Correction → Final Predictions
```
...
### NN2 Output Mapping (Gradient Boosting Solution)

**CRITICAL FIX**: NN2 outputs in scaled space are out-of-distribution for the original scaler's inverse transform, causing incorrect ppb conversions.

#### Problem Identified

- **NN2 performs excellently in scaled space**: 35% improvement over PINN
- **NN2 degrades in original ppb space**: -168% degradation when using `scalers['sensors'].inverse_transform()`
- **Root cause**: NN2 outputs values in scaled space (range: -5.90 to 8.31) that are outside the scaler's training range (approximately -0.16 to 3.20)

#### Solution: Gradient Boosting Mapping Model

The NN2 model outputs predictions in **scaled space** (not ppb). After the NN2 model runs, we use a **Gradient Boosting Regressor** that learns the direct mapping from NN2 scaled outputs to actual ppb values. This mapping is applied as a post-processing step, not built into the NN2 model itself.

**Implementation**:
- **Script**: `realtime/implement_nn2_output_mapping.py`
- **Model Location**: `realtime/nn2_timefix/nn2_output_to_ppb_mapping.pkl`
- **Helper Module**: `realtime/nn2_mapping_utils.py`

**Model Details**:
- **Type**: GradientBoostingRegressor
- **Parameters**: n_estimators=100, max_depth=5
- **Training Data**: 34,213 non-zero samples from full training dataset
- **Performance**:
  - **MAE**: 0.2529 ppb (vs 1.2256 ppb with original scaler)
  - **R²**: 0.9717
  - **Improvement**: 79.4% over baseline
- **Overfitting Check**: Train MAE 0.2512, Test MAE 0.2620 (no overfitting)
...
# Convert NN2 scaled outputs to ppb
...
**Zero Handling**:
- Only non-zero values are transformed (zeros remain zero)
- Uses threshold: `np.abs(corrected_scaled_np) > 1e-6`

**Performance Results (2019 Full Year)**:
- **Before (original scaler)**: NN2 MAE = 1.1073 ppb (-109% degradation)
- **After (Gradient Boosting mapping)**: NN2 MAE = 0.2649 ppb (**50.0% improvement**)
- **PINN MAE**: 0.5296 ppb
- **NN2 Improvement**: 50.0% over PINN

**Monthly Performance (2019)**:
- January: 40.6% improvement
- February: 41.1% improvement
- March: 72.5% improvement
- April: 51.9% improvement
- May: 50.1% improvement
- June: 47.5% improvement
- July: 45.9% improvement
- August: 61.3% improvement
- September: 36.1% improvement
- October: 34.6% improvement
- November: 42.4% improvement
- December: 36.6% improvement

**Inference Performance**:
- Single value: ~0.077 ms per value (~13,000 predictions/sec)
- Batch: ~0.0007 ms per value (~1.35M predictions/sec)
- Acceptable for real-time pipeline (processes every 15 minutes)

**Files Updated**:
- `realtime/concentration_predictor.py` - Real-time pipeline
- `realtime/validate_nn2_january_2019.py` - Validation scripts
- `realtime/validate_jan_mar_2021.py` - Validation scripts
- `realtime/test_pipeline_2019.py` - Full pipeline test
- `realtime/validate_jan_2021_updated_nn2.py` - Validation scripts
- `realtime/simpletesting/benzene_pipeline.py` - Legacy pipeline
...
class ConcentrationPredictor:
    def __init__(self, grid_resolution: int = 100):
        ...
        self.nn2_path = "/Users/neevpratap/Desktop/benzenepipelinev2/realtime/nn2_timefix/nn2_master_model_spatial-3.pth"
        self.nn2_scalers_path = "/Users/neevpratap/Desktop/benzenepipelinev2/realtime/nn2_timefix/nn2_master_scalers-2.pkl"
...
### Inverse Transform (CRITICAL - Gradient Boosting Mapping)

**IMPORTANT**: As of the latest fix, we use a Gradient Boosting mapping model instead of the original scaler's inverse transform. See [NN2 Output Mapping](#nn2-output-mapping-gradient-boosting-solution) section for details.

```python
from nn2_mapping_utils import nn2_scaled_to_ppb

# Inverse transform using Gradient Boosting mapping model (more accurate)
corrected_scaled_np = corrected_scaled.cpu().numpy().flatten()

if self.nn2_mapping_model is not None:
    # Use mapping model (fixes out-of-distribution issue)
    sensor_corrected = nn2_scaled_to_ppb(corrected_scaled_np, mapping_model=self.nn2_mapping_model)
else:
...
### Script Location
`/Users/neevpratap/Desktop/benzenepipelinev2/realtime/validate_nn2_january_2019.py`

### Process

1. **Load Models**: PINN and NN2 (from nn2_timefix)
2. **Load Sensor Data**: Ground truth readings
3. **Load Facility Data**: Meteorological data
4. **For each sensor reading timestamp (t+3):**
   - Get met data from t (3 hours before)
   - Compute PINN predictions using met data from t, forecasting for t+3
   - Get actual sensor readings from t+3 (for current_sensors input)
   - Apply NN2 correction
   - Compare with actual sensor readings at t+3
...
- Current sensors: Actual readings at t+3 (same as target)
...
realtime/nn2_timefix/
├── nn2_master_model_spatial-3.pth    # NN2 model
├── nn2_master_scalers-2.pkl          # NN2 scalers
└── leave_one_out_results_spatial-3.json  # Training results
```
...
**Usage in Pipeline**:
1. **NN2 Training**: 
   - Used as target values (actual readings at timestamp t+3)
   - Used as current_sensors input (actual readings at timestamp t+3)
   - Aligned with PINN predictions at same timestamps

2. **Validation**:
   - Used as ground truth to compare against PINN and NN2 predictions
...
3. Compute PINN across full domain:
...
4. Apply NN2 correction:
   a. Compute PINN at 9 sensor locations directly (not interpolate)
      - For each facility, for each sensor:
        * Run PINN directly at sensor coordinates
        * Use simulation time t=3.0 hours
        * Superimpose
   b. Scale inputs (handle zeros correctly)
   c. Run NN2 model
   d. Inverse transform using Gradient Boosting mapping model (handle zeros correctly)
   e. Calculate corrections: NN2 - PINN at sensors
   f. Interpolate corrections across domain using RBF
   g. Apply distance-based confidence weighting
   h. Add weighted corrections to PINN field
...
3. **Zero Handling**: Only scale/inverse-transform non-zero values
4. **Direct Computation**: Compute PINN directly at sensor locations, don't interpolate
5. **Superimposition**: Process each facility separately, then superimpose
6. **Unit Conversion**: Always use `313210039.9` for kg/m² to ppb conversion
7. **Normalization Ranges**: Must match exactly the values specified in this document
8. **Current Sensors**: In training, current_sensors = target = actual readings at t+3
...
- [ ] Zero values are masked before scaling/inverse scaling
...
- [ ] Current sensors input matches training structure (readings at t+3)
...
### NN2 Training Configuration

**Full Configuration** (from `nn2.py`):
```python
CONFIG = {
    ...
    'learning_rate': 0.001,
    'epochs': 100,
    'lambda_correction': 0.001,  # Regularization weight for corrections
...
**NOTE**: This function now uses the Gradient Boosting mapping model for inverse transform. See [NN2 Output Mapping](#nn2-output-mapping-gradient-boosting-solution) section.
...
def apply_nn2_correction(nn2, scalers, sensor_coords_spatial, pinn_values, meteo_data, timestamp, current_sensor_readings, nn2_mapping_model=None):
    ...
    # Inverse transform using Gradient Boosting mapping model (CRITICAL FIX)
    corrected_scaled_np = corrected_scaled.cpu().numpy().flatten()
    
    if nn2_mapping_model is not None:
        # Use Gradient Boosting mapping model (fixes out-of-distribution issue)
        nn2_corrected = nn2_scaled_to_ppb(corrected_scaled_np, mapping_model=nn2_mapping_model)
    else:
        # Fallback to original scaler (if mapping model not available)
...
3. **Zero Handling**: Only scale/inverse-transform non-zero values
4. **Direct Computation**: Compute PINN directly at sensors (don't interpolate)
5. **Superimposition**: Process each facility separately, then superimpose
6. **Unit Conversion**: Always use `313210039.9` for kg/m² to ppb
7. **Normalization Ranges**: Must match exactly the values specified
8. **Current Sensors**: In training, current_sensors = target = actual readings at t+3
9. **Temporal Features**: 6 features, NOT scaled
10. **RBF Interpolation**: Used for extending corrections from sensors to full domain
11. **Gradient Boosting Mapping**: **CRITICAL** - Use `nn2_scaled_to_ppb()` with mapping model instead of `scalers['sensors'].inverse_transform()` for accurate NN2 output conversion (50% improvement over PINN)
...
**Total Lines**: 1400+
**Last Updated**: 2024 (Updated with Gradient Boosting Mapping Solution)
